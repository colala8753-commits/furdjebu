...
do
	...
	local function startAutoFarmLoop()
		...
		task.spawn(function()
			-- ===== SPEED — реальное движение по полу (игра считает шаги) =====
			if farmWorld == "Speed" then
				...
				-- НЕ трогаем WalkSpeed (игра сама управляет скоростью)
				-- НЕ летим, НЕ noclip — только Move по полу
				setNoClip(false)
				clearFarmBV()
				...
				-- каждый кадр двигаем персонажа — иначе игра не считает уровень
				local moveConn
				moveConn = RunService.Heartbeat:Connect(function()
					...
					if dist > 0.1 then
						local dir = to.Unit
						-- Humanoid:Move — реальное движение, игра считает
						pcall(function()
							hum:Move(dir, false)
						end)
					end
	...
	local function setAntiKnock(state)
		...
		if state then
			-- НЕ трогаем AssemblyLinearVelocity игрока (в этой игре скорость огромная)
			-- только сносим чужие физические силы откидывания
			antiKnockConnection = RunService.Heartbeat:Connect(function()
				...
				for _, obj in pairs(hrp:GetChildren()) do
					...
					if cn == "BodyVelocity" or cn == "BodyForce" or cn == "BodyPosition"
						or cn == "BodyAngularVelocity" or cn == "LinearVelocity"
						or cn == "VectorForce" or cn == "AngularVelocity"
						or cn == "AlignPosition" or cn == "AlignOrientation" then
						if obj.Name ~= "FurdjeFarmBV" and obj ~= flyBV and obj.Name ~= "FurdjeFlyBV" and obj.Name ~= "FurdjeFlyBG" then
							pcall(function() obj:Destroy() end)
						end
					end
				end
	...
	-- только сладости / крендели на стенах (НЕ стены, пол, дорожку)
	local CANDY_DECOR = {
		"pretzel", "candy", "candycane", "candy_cane", "chocolate", "choco",
		"lollipop", "lolly", "donut", "doughnut", "muffin", "cookie",
		"marshmallow", "gummy", "sprinkle", "macaron", "macaroon",
		"sweet", "cupcake", "icecream", "ice_cream", "wafer", "caramel",
		"gumball", "jelly", "bonbon", "truffle", "croissant", "biscuit",
		"peppermint", "gumdrop", "taffy", "fudge", "brownie",
	}

	local function isCandyDecorName(name)
		if not name then return false end
		local n = string.lower(name)
		-- не трогаем служебные имена карты
		if n == "part" or n == "meshpart" or n == "spawn" or n == "baseplate"
			or n == "floor" or n == "wall" or n == "ground" or n == "platform"
			or n == "stage" or n == "checkpoint" or n == "win" or n == "kill"
			or string.find(n, "stage", 1, true) or string.find(n, "spawn", 1, true) then
			return false
		end
		for _, d in ipairs(CANDY_DECOR) do
			if string.find(n, d, 1, true) then return true end
		end
		if isKeycapName and isKeycapName(name) then return true end
		return false
	end
	...
	local function removeAllDecor()
		task.spawn(function()
			local count = 0
			for _, v in pairs(workspace:GetDescendants()) do
				-- только объекты с именем сладости
				if isCandyDecorName(v.Name) then
					pcall(function()
						-- не удаляем персонажей
		...
		if state then
			...
			if not decorConnection then
				decorConnection = workspace.DescendantAdded:Connect(function(desc)
					if fpsBoostEnabled and isCandyDecorName(desc.Name) then
						task.defer(function()
							if not desc or not desc.Parent then return end
