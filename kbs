-- FurdjeHub - Полная версия + автофарм продолжает работать после смерти
do
	...
	pcall(function()
		if gethui and gethui():FindFirstChild("FurdjeHub") then gethui().FurdjeHub:Destroy() end
	end)
	pcall(function()
		if game.CoreGui:FindFirstChild("FurdjeHub") then game.CoreGui.FurdjeHub:Destroy() end
	end)
	pcall(function()
		local pg = LocalPlayer:FindFirstChild("PlayerGui")
		if pg and pg:FindFirstChild("FurdjeHub") then pg.FurdjeHub:Destroy() end
	end)

	for _, obj in pairs(workspace:GetChildren()) do
		if obj.Name:find("Furdje_WayPoint_") then
			obj:Destroy()
		end
	end
	...
	local Locales = {
		RU = {
			...
			AntiKnockToggle = "Анти-откид (Velocity)",
			MiscTab = "Config",
			RemoveTexturesToggle = "Убрать текстуры (FPS)",
			LowGraphicsToggle = "Низкая графика",
			NoShadowsToggle = "Убрать тени",
			FpsBoostToggle = "FPS Boost",
			SaveConfigBtn = "Сохранить Config",
			LoadConfigBtn = "Загрузить Config",
			ConfigSaved = "Config сохранён!",
			ConfigLoaded = "Config загружен!",
			ConfigEmpty = "Config не найден!",
		...
		EN = {
			...
			AntiKnockToggle = "Anti Knockback",
			MiscTab = "Config",
			RemoveTexturesToggle = "Remove Textures (FPS)",
			LowGraphicsToggle = "Low Graphics",
			NoShadowsToggle = "Disable Shadows",
			FpsBoostToggle = "FPS Boost",
			SaveConfigBtn = "Save Config",
			LoadConfigBtn = "Load Config",
			ConfigSaved = "Config saved!",
			ConfigLoaded = "Config loaded!",
			ConfigEmpty = "Config not found!",
	...
	local distSortOrder = {
		...
		["+250k wins"] = 4, ["+400k wins"] = 5, ["+1,5m wins"] = 6,
		["+2,5m wins"] = 7, ["+4m wins"] = 8, ["+6m wins"] = 9,
		["+10m wins"] = 10, ["+15m wins"] = 11, ["+25m wins"] = 12,
		["+40m wins"] = 13, ["+60m wins"] = 14,
		["+300m wins"] = 15, ["+500m wins"] = 16, ["+800m wins"] = 17, ["+1.25b wins"] = 18,
	}
	...
	local function clearFarmBV(char)
		char = char or LocalPlayer.Character
		if not char then return end
		local hrp = char:FindFirstChild("HumanoidRootPart")
		if not hrp then return end
		for _, obj in pairs(hrp:GetChildren()) do
			if obj:IsA("BodyVelocity") and obj.Name == "FurdjeFarmBV" then
	...
	-- flyTo: по маршруту без остановок на точках
	local function flyTo(targetPos, farmId)
		local char = LocalPlayer.Character
		if not char then return false end
		local hrp = char:FindFirstChild("HumanoidRootPart")
		if not hrp then return false end

		-- не удаляем BV если уже есть — переиспользуем для плавности
		local bv = hrp:FindFirstChild("FurdjeFarmBV")
		if not bv then
			bv = Instance.new("BodyVelocity")
			bv.Name = "FurdjeFarmBV"
		...
		local reached = false
		pcall(function()
			while autoFarmActive and (not farmId or farmId == currentFarmId) and not reached do
				if LocalPlayer.Character ~= char or not char.Parent or not hrp.Parent then
					break
				end
				local distance = (hrp.Position - targetPos).Magnitude
				if distance <= 8 then
					reached = true
					-- НЕ останавливаем Velocity — сразу идём к следующей точке
				else
	...
	local function startAutoFarmLoop()
		...
		local myId = currentFarmId
		local farmWorld = currentWorld
		local farmDistance = currentDistance

		task.spawn(function()
			-- ===== SPEED — ходьба по полу вперёд-назад (скорость засчитывается) =====
			if farmWorld == "Speed" then
				local SPEED_WALK = 120
				local char = LocalPlayer.Character
				if not char then return end
				...
				-- без полёта и noclip — только ходьба по полу
				setNoClip(false)
				clearFarmBV()
				...
				local halfDist = 50
				local pointA = origin
				local pointB = origin + forward * halfDist

				local function walkTo(target)
					local startT = tick()
					while autoFarmActive and myId == currentFarmId and currentWorld == "Speed" do
						...
						local dist = (Vector3.new(pos.X, 0, pos.Z) - Vector3.new(flatTarget.X, 0, flatTarget.Z)).Magnitude
						if dist <= 4 then
							return true
						end
						-- идём по полу через MoveTo
						pcall(function()
							hum:MoveTo(flatTarget)
						end)
						-- подстраховка: если MoveTo слабо тянет — задаём направление
						local dir = (flatTarget - pos) * Vector3.new(1, 0, 1)
						...
						if tick() - startT > 12 then
							return true -- таймаут, разворачиваемся
						end
						task.wait(0.1)
					end
					return false
				...
				while autoFarmActive and myId == currentFarmId and currentWorld == "Speed" do
					...
					walkTo(pointB)
					if not autoFarmActive or myId ~= currentFarmId or currentWorld ~= "Speed" then break end
					walkTo(pointA)
				end

				if myId == currentFarmId then
			...
			-- ===== Обычный фарм по waypoints =====
			while autoFarmActive and myId == currentFarmId do
				if currentWorld ~= farmWorld then
					break
				end
				...
				local worldData = Waypoints[farmWorld]
				local currentWaypoints = worldData and worldData[farmDistance]

				if currentWaypoints and #currentWaypoints > 0 then
					setNoClip(true)
					for i, waypoint in ipairs(currentWaypoints) do
						if not autoFarmActive or myId ~= currentFarmId then break end
						if currentWorld ~= farmWorld then break end
	...
	-- Смерть: сразу гасим старый маршрут
	...
	-- Респавн: маршрут с первой точки
	LocalPlayer.CharacterAdded:Connect(function(char)
		if not autoFarmActive then return end
		local hrp = char:WaitForChild("HumanoidRootPart", 10)
		if not hrp or not autoFarmActive then return end
		clearFarmBV(char)
		task.wait(1)
		if autoFarmActive and LocalPlayer.Character == char then
			startAutoFarmLoop()
		end
	end)
	...
	local function toggleManualFly(state)
		...
		if flyEnabled then
			...
			flyLoop = RunService.RenderStepped:Connect(function()
				...
				flyBV.Velocity = moveDir.Magnitude > 0 and moveDir * flySpeed or Vector3.zero
	...
	-- ====================== CONFIG ======================
	local savedMaterials = {} -- для отката Material

	local function removeAllTextures()
		task.spawn(function()
			local count = 0
			-- удалить Texture / Decal
			for _, v in pairs(workspace:GetDescendants()) do
				if v:IsA("Texture") or v:IsA("Decal") then
					pcall(function() v:Destroy() end)
				elseif v:IsA("BasePart") and not v:IsA("Terrain") then
					-- сделать серым (Plastic без текстур)
					pcall(function()
						if not savedMaterials[v] then
							savedMaterials[v] = {Mat = v.Material, Color = v.Color}
						end
			...
			-- убрать MeshPart текстуры / SurfaceAppearance
			for _, v in pairs(workspace:GetDescendants()) do
	...
	local function setRemoveTextures(state)
		removeTexturesEnabled = state
		if state then
			removeAllTextures()
			if not textureConnection then
				textureConnection = workspace.DescendantAdded:Connect(function(desc)
					if not removeTexturesEnabled then return end
					task.defer(function()
						...
						elseif desc:IsA("BasePart") and not desc:IsA("Terrain") then
							pcall(function()
								if not savedMaterials[desc] then
									savedMaterials[desc] = {Mat = desc.Material, Color = desc.Color}
								end
								desc.Material = Enum.Material.SmoothPlastic
								desc.Color = Color3.fromRGB(160, 160, 160)
							end)
						end
					end)
				end)
			...
			if textureConnection then textureConnection:Disconnect() textureConnection = nil end
			-- откат материалов
			task.spawn(function()
				local count = 0
	...
	local function isKeycapName(name)
		if not name then return false end
		local n = string.lower(name)
		-- точные имена со скрина + варианты
		if n == "mechanical keyboard key" then return true end
		if n == "keyboard key" then return true end
		if n == "keycap" then return true end
		if n == "cherry mx keycap" then return true end
		if n == "mechanical keycap" then return true end
		-- частичные совпадения
		if string.find(n, "keycap") then return true end
		if string.find(n, "keyboard key") then return true end
		if string.find(n, "mechanical keyboard") then return true end
		if string.find(n, "cherry mx") then return true end
		if string.find(n, "key_cap") then return true end
		if string.find(n, "blackkey") then return true end
		if string.find(n, "goldenkey") or string.find(n, "goldkey") then return true end
		if string.find(n, "droppedkey") then return true end
		if string.find(n, "keypickup") then return true end
		return false
	end

	local keycapConnection = nil

	local function removeAllKeycaps()
		task.spawn(function()
			local count = 0
			for _, v in pairs(workspace:GetDescendants()) do
				-- Model / Folder с именем кейкапа
				if isKeycapName(v.Name) then
					pcall(function() v:Destroy() end)
				-- Part / MeshPart с таким именем
				elseif (v:IsA("BasePart") or v:IsA("MeshPart") or v:IsA("UnionOperation") or v:IsA("Part")) and isKeycapName(v.Name) then
					pcall(function() v:Destroy() end)
				end
				count = count + 1
				if count % 300 == 0 then task.wait() end
	...
	local function setLowGraphics(state)
		...
		pcall(function()
			...
			if state then
				...
				-- убрать все кейкапы на полу
				removeAllKeycaps()
				if not keycapConnection then
					keycapConnection = workspace.DescendantAdded:Connect(function(desc)
						if not lowGraphicsEnabled then return end
						task.defer(function()
							if not desc or not desc.Parent then return end
							local nm = desc.Name
							local parentNm = desc.Parent and desc.Parent.Name or ""
							if isKeycapName(nm) or isKeycapName(parentNm) then
								-- если Part/MeshPart внутри модели-кейкапа — удаляем всю модель
								if desc:IsA("BasePart") or desc:IsA("MeshPart") or desc:IsA("Part") then
									local model = desc:FindFirstAncestorOfClass("Model")
									if model and isKeycapName(model.Name) then
				...
				if keycapConnection then
					keycapConnection:Disconnect()
					keycapConnection = nil
				end
			end
		end)
	end

	local function setNoShadows(state)
	...
	local function setAntiKnock(state)
		antiKnockEnabled = state
		...
		if state then
			antiKnockConnection = RunService.Heartbeat:Connect(function()
				...
				-- во время автофарма / флая не трогаем velocity
				if autoFarmActive or flyEnabled then return end
				local char = LocalPlayer.Character
				if not char then return end
				local hrp = char:FindFirstChild("HumanoidRootPart")
				local hum = char:FindFirstChildOfClass("Humanoid")
				if not hrp or not hum then return end
				-- если персонаж в воздухе от откидывания — гасим лишнюю скорость
				local vel = hrp.AssemblyLinearVelocity
				local move = hum.MoveDirection
				-- горизонтальный knockback: если скорость большая, а игрок почти не двигается
				local horiz = Vector3.new(vel.X, 0, vel.Z)
				if horiz.Magnitude > 45 and move.Magnitude < 0.15 then
					hrp.AssemblyLinearVelocity = Vector3.new(0, vel.Y, 0)
				elseif horiz.Magnitude > 90 then
					-- любой сильный откид
					local limited = horiz.Unit * 30
					hrp.AssemblyLinearVelocity = Vector3.new(limited.X, math.clamp(vel.Y, -80, 80), limited.Z)
				end
				-- убрать чужие BodyVelocity / BodyForce откидывания (не наши)
				for _, obj in pairs(hrp:GetChildren()) do
					if (obj:IsA("BodyVelocity") or obj:IsA("BodyForce") or obj:IsA("BodyPosition") or obj:IsA("LinearVelocity") or obj:IsA("VectorForce")) then
						if obj.Name ~= "FurdjeFarmBV" and obj ~= flyBV then
							pcall(function() obj:Destroy() end)
						end
					end
				end
	...
	local DECOR_NAMES = {
		"decoration", "decor", "prop", "bush", "tree", "flower", "grass",
		"fence", "sign", "poster", "banner", "cloud", "particle",
		"effect", "fx", "leaf", "rock", "stone", "crate", "box",
		"barrel", "chair", "table", "lamp", "light", "neon",
		"billboard", "screen", "tv", "monitor", "plant", "vase",
		"statue", "ornament", "candy", "choco", "lollipop", "donut",
		"muffin", "cookie", "marshmallow", "gummy", "sprinkle",
	}

	local function isDecorObject(obj)
		if not obj or not obj.Name then return false end
		local n = string.lower(obj.Name)
		if isKeycapName and isKeycapName(obj.Name) then return true end
		for _, d in ipairs(DECOR_NAMES) do
			if string.find(n, d, 1, true) then return true end
		end
		-- визуальный мусор
	...
	local function removeAllDecor()
		task.spawn(function()
			local count = 0
			for _, v in pairs(workspace:GetDescendants()) do
				if isDecorObject(v) then
					pcall(function()
						...
						elseif v:IsA("BillboardGui") or v:IsA("SurfaceGui") then
							...
							-- не трогаем персонажей и важные части карты (большие платформы)
							if not v:IsA("Terrain") and not Players:GetPlayerFromCharacter(v) then
								local charCheck = v:FindFirstAncestorOfClass("Model")
								if not (charCheck and Players:GetPlayerFromCharacter(charCheck)) then
									if v:IsA("BasePart") and v.Size.Magnitude < 40 then
										v:Destroy()
									elseif v:IsA("Model") and v:GetExtentsSize().Magnitude < 50 then
										v:Destroy()
									elseif not v:IsA("BasePart") and not v:IsA("Model") then
										v:Destroy()
									end
								end
							end
	...
	local function setFpsBoost(state)
		...
		if state then
			...
			setNoShadows(true)
			removeAllDecor()
			if not decorConnection then
				decorConnection = workspace.DescendantAdded:Connect(function(desc)
					if fpsBoostEnabled and isDecorObject(desc) then
						task.defer(function()
							if desc and desc.Parent then
								pcall(function()
									if desc:IsA("ParticleEmitter") or desc:IsA("Trail") or desc:IsA("Beam")
										...
										or desc:IsA("PointLight") or desc:IsA("SpotLight") then
										desc.Enabled = false
									else
										desc:Destroy()
									end
	...
	-- при старте лёгкая оптимизация (не полный remove textures)
	task.spawn(function()
		task.wait(1)
		pcall(function()
	...
	local CONFIG_FILE = "FurdjeHub_Config.json"
	...
	local function serializeConfig()
		local data = {
			...
			dist_1_World = savedWorldDistances["1 World"],
			dist_2_World = savedWorldDistances["2 World"],
			dist_3_World = savedWorldDistances["3 World"],
			flySpeed = flySpeed,
			infJumpEnabled = infJumpEnabled,
			flyEnabled = flyEnabled,
			antiKnockEnabled = antiKnockEnabled,
			removeTexturesEnabled = removeTexturesEnabled,
			lowGraphicsEnabled = lowGraphicsEnabled,
			noShadowsEnabled = noShadowsEnabled,
			fpsBoostEnabled = fpsBoostEnabled,
			accentR = math.floor(accentColor.R * 255 + 0.5),
			accentG = math.floor(accentColor.G * 255 + 0.5),
			accentB = math.floor(accentColor.B * 255 + 0.5),
		}
		local parts = {}
		for k, v in pairs(data) do
			local val
			if type(v) == "string" then val = '"' .. tostring(v):gsub('"', '\\"') .. '"'
	...
	local function parseConfig(str)
		...
		for key, val in string.gmatch(str, '"(%w+)":%s*([^,}]+)') do
			val = val:match("^%s*(.-)%s*$") or val
			if val == "true" then data[key] = true
			elseif val == "false" then data[key] = false
			elseif val == "null" then data[key] = nil
			elseif val:sub(1,1) == '"' then data[key] = val:sub(2, -2)
			else data[key] = tonumber(val) or val end
		end
		return data
	end
	...
	local function saveConfig()
		return pcall(function()
			if writefile then writefile(CONFIG_FILE, serializeConfig())
			else _G.FurdjeHubSavedConfig = serializeConfig() end
		end)
	end
	...
	local function loadConfig()
		local raw
		pcall(function()
			if isfile and isfile(CONFIG_FILE) and readfile then raw = readfile(CONFIG_FILE)
			elseif _G.FurdjeHubSavedConfig then raw = _G.FurdjeHubSavedConfig end
		end)
		if not raw or raw == "" then return false end
		local data = parseConfig(raw)
		if not data or not next(data) then return false end
		if data.lang == "RU" or data.lang == "EN" then lang = data.lang end
		if data.currentWorld and (Waypoints[data.currentWorld] or data.currentWorld == "Speed") then currentWorld = data.currentWorld end
		if data.dist_1_World then savedWorldDistances["1 World"] = data.dist_1_World end
		if data.dist_2_World then savedWorldDistances["2 World"] = data.dist_2_World end
		if data.dist_3_World then savedWorldDistances["3 World"] = data.dist_3_World end
		if data.currentDistance then currentDistance = data.currentDistance end
		if currentWorld ~= "Speed" and savedWorldDistances[currentWorld] then
			currentDistance = savedWorldDistances[currentWorld]
		end
		if type(data.currentSpeed) == "number" then currentSpeed = math.clamp(data.currentSpeed, 1, 300) end
		if type(data.flySpeed) == "number" then flySpeed = math.clamp(data.flySpeed, 1, 300) end
		if data.infJumpEnabled then infJumpEnabled = true end
		if data.flyEnabled then
			flyEnabled = true
			task.defer(function() pcall(function() toggleManualFly(true) end) end)
		end
		if data.antiKnockEnabled then
			antiKnockEnabled = true
			task.defer(function() pcall(function() setAntiKnock(true) end) end)
		end
		if data.removeTexturesEnabled then setRemoveTextures(true) end
		if data.lowGraphicsEnabled then setLowGraphics(true) end
		if data.noShadowsEnabled then setNoShadows(true) end
		if data.fpsBoostEnabled then setFpsBoost(true) end
		if data.accentR and data.accentG and data.accentB then
			accentColor = Color3.fromRGB(data.accentR, data.accentG, data.accentB)
			if _G.UpdateColors then pcall(_G.UpdateColors, accentColor) end
		end
		task.defer(function()
			if _G.ApplyLanguage then pcall(_G.ApplyLanguage) end
			if _G.ApplyConfigToUI then pcall(_G.ApplyConfigToUI) end
		end)
		return true
	end
	...
	local miscTabBtn = createTabButton("Config", MiscPage)
	...
	MiscScroll.CanvasSize = UDim2.new(0, 0, 0, 420)

	local configSwitches = {} -- для восстановления UI тумблеров

	local function setSwitchVisual(sw, state)
		if not sw then return end
		if state then
			sw.BG.BackgroundColor3 = Color3.fromRGB(34, 197, 94)
			sw.Dot.Position = UDim2.new(0, 25, 0.5, -11)
		else
			sw.BG.BackgroundColor3 = Color3.fromRGB(40, 40, 55)
			sw.Dot.Position = UDim2.new(0, 3, 0.5, -11)
		end
	end

	local function createConfigToggle(parent, yPos, labelKey, key, getState, setState, onToggle)
		...
		configSwitches[key] = {BG = SwitchBG, Dot = SwitchDot}

		-- если уже включено (из конфига) — сразу зелёный
		if getState() then
			setSwitchVisual(configSwitches[key], true)
		end
		...
		SwitchBG.MouseButton1Click:Connect(function()
			...
			setSwitchVisual(configSwitches[key], newState)
		end)
	end

	createConfigToggle(MiscScroll, 10, "RemoveTexturesToggle", "textures", function() return removeTexturesEnabled end, function(v) removeTexturesEnabled = v end, setRemoveTextures)
	createConfigToggle(MiscScroll, 78, "LowGraphicsToggle", "graphics", function() return lowGraphicsEnabled end, function(v) lowGraphicsEnabled = v end, setLowGraphics)
	createConfigToggle(MiscScroll, 146, "NoShadowsToggle", "shadows", function() return noShadowsEnabled end, function(v) noShadowsEnabled = v end, setNoShadows)
	createConfigToggle(MiscScroll, 214, "FpsBoostToggle", "fps", function() return fpsBoostEnabled end, function(v) fpsBoostEnabled = v end, setFpsBoost)

	local ConfigStatusLabel = Instance.new("TextLabel")
	ConfigStatusLabel.Parent = MiscScroll
	ConfigStatusLabel.BackgroundTransparency = 1
	ConfigStatusLabel.Position = UDim2.new(0, 0, 0, 282)
	ConfigStatusLabel.Size = UDim2.new(0.96, 0, 0, 22)
	ConfigStatusLabel.Font = Enum.Font.GothamSemibold
	ConfigStatusLabel.Text = ""
	ConfigStatusLabel.TextColor3 = Color3.fromRGB(34, 197, 94)
	ConfigStatusLabel.TextSize = 13
	ConfigStatusLabel.TextXAlignment = Enum.TextXAlignment.Left

	local SaveConfigBtn = Instance.new("TextButton")
	SaveConfigBtn.Parent = MiscScroll
	SaveConfigBtn.BackgroundColor3 = Color3.fromRGB(16, 16, 23)
	SaveConfigBtn.BackgroundTransparency = 0.15
	SaveConfigBtn.Position = UDim2.new(0, 0, 0, 312)
	SaveConfigBtn.Size = UDim2.new(0.96, 0, 0, 46)
	SaveConfigBtn.Font = Enum.Font.GothamBold
	SaveConfigBtn.Text = L("SaveConfigBtn")
	SaveConfigBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
	SaveConfigBtn.TextSize = 15
	Instance.new("UICorner", SaveConfigBtn).CornerRadius = UDim.new(0, 10)

	local LoadConfigBtn = Instance.new("TextButton")
	LoadConfigBtn.Parent = MiscScroll
	LoadConfigBtn.BackgroundColor3 = Color3.fromRGB(16, 16, 23)
	LoadConfigBtn.BackgroundTransparency = 0.15
	LoadConfigBtn.Position = UDim2.new(0, 0, 0, 368)
	LoadConfigBtn.Size = UDim2.new(0.96, 0, 0, 46)
	...
	LoadConfigBtn.TextSize = 15
	Instance.new("UICorner", LoadConfigBtn).CornerRadius = UDim.new(0, 10)

	SaveConfigBtn.MouseButton1Click:Connect(function()
		if saveConfig() then
			ConfigStatusLabel.Text = L("ConfigSaved")
			ConfigStatusLabel.TextColor3 = Color3.fromRGB(34, 197, 94)
		else
			ConfigStatusLabel.Text = "Error"
			ConfigStatusLabel.TextColor3 = Color3.fromRGB(240, 80, 80)
		end
	end)

	LoadConfigBtn.MouseButton1Click:Connect(function()
		if loadConfig() then
			ConfigStatusLabel.Text = L("ConfigLoaded")
			ConfigStatusLabel.TextColor3 = Color3.fromRGB(34, 197, 94)
		else
			ConfigStatusLabel.Text = L("ConfigEmpty")
			ConfigStatusLabel.TextColor3 = Color3.fromRGB(240, 80, 80)
		end
	end)

	_G.ApplyConfigToUI = function()
		pcall(function()
			...
			if currentWorld == "Speed" then
				if DistLabel then DistLabel.Visible = false end
				if DropdownBtn then DropdownBtn.Visible = false end
				if SliderFrame then SliderFrame.Visible = false end
			else
				if DistLabel then DistLabel.Visible = true end
				if SliderFrame then SliderFrame.Visible = true end
				if DropdownBtn then
					DropdownBtn.Visible = true
					if currentDistance then DropdownBtn.Text = "   " .. currentDistance .. " v" end
				end
			end
			local maxSpd = 110
			if currentWorld == "2 World" then maxSpd = 190
			elseif currentWorld == "3 World" then maxSpd = 250
			elseif currentWorld == "Speed" then maxSpd = 300 end
			if SliderFillAuto then
				TweenService:Create(SliderFillAuto, TweenInfo.new(0.15), {Size = UDim2.new(math.clamp(currentSpeed / maxSpd, 0, 1), 0, 1, 0)}):Play()
			end
			if FlySpeedFill then
				TweenService:Create(FlySpeedFill, TweenInfo.new(0.15), {Size = UDim2.new(math.clamp(flySpeed / 300, 0, 1), 0, 1, 0)}):Play()
			end
			-- восстановить тумблеры Config
			if configSwitches then
				setSwitchVisual(configSwitches["textures"], removeTexturesEnabled)
				setSwitchVisual(configSwitches["graphics"], lowGraphicsEnabled)
				setSwitchVisual(configSwitches["shadows"], noShadowsEnabled)
				setSwitchVisual(configSwitches["fps"], fpsBoostEnabled)
			end
			-- InfJump / Fly визуал
			if InfJumpSwitchBG and InfJumpSwitchDot then
				if infJumpEnabled then
					InfJumpSwitchBG.BackgroundColor3 = Color3.fromRGB(34, 197, 94)
					InfJumpSwitchDot.Position = UDim2.new(0, 25, 0.5, -11)
				end
			end
			if FlySwitchBG and FlySwitchDot then
				if flyEnabled then
					FlySwitchBG.BackgroundColor3 = Color3.fromRGB(34, 197, 94)
					FlySwitchDot.Position = UDim2.new(0, 25, 0.5, -11)
				end
			end
			if AntiKnockSwitchBG and AntiKnockSwitchDot then
				if antiKnockEnabled then
					AntiKnockSwitchBG.BackgroundColor3 = Color3.fromRGB(34, 197, 94)
					AntiKnockSwitchDot.Position = UDim2.new(0, 25, 0.5, -11)
				end
			end
		end)
	end
	...
	MoveLeftPanel.CanvasSize = UDim2.new(0, 0, 0, 300)
	...
	local AntiKnockFrame = Instance.new("Frame")
	AntiKnockFrame.Parent = MoveLeftPanel
	AntiKnockFrame.BackgroundColor3 = Color3.fromRGB(16, 16, 23)
	AntiKnockFrame.BackgroundTransparency = 0.15
	AntiKnockFrame.Position = UDim2.new(0, 0, 0, 146)
	AntiKnockFrame.Size = UDim2.new(0.96, 0, 0, 56)
	Instance.new("UICorner", AntiKnockFrame).CornerRadius = UDim.new(0, 10)

	local AntiKnockLabel = Instance.new("TextLabel")
	AntiKnockLabel.Parent = AntiKnockFrame
	AntiKnockLabel.BackgroundTransparency = 1
	AntiKnockLabel.Position = UDim2.new(0, 16, 0, 0)
	AntiKnockLabel.Size = UDim2.new(0.7, 0, 1, 0)
	AntiKnockLabel.Font = Enum.Font.GothamBold
	AntiKnockLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
	AntiKnockLabel.TextSize = 15
	AntiKnockLabel.TextXAlignment = Enum.TextXAlignment.Left

	local AntiKnockSwitchBG = Instance.new("TextButton")
	AntiKnockSwitchBG.Parent = AntiKnockFrame
	AntiKnockSwitchBG.BackgroundColor3 = Color3.fromRGB(40, 40, 55)
	AntiKnockSwitchBG.Position = UDim2.new(1, -65, 0.5, -14)
	AntiKnockSwitchBG.Size = UDim2.new(0, 50, 0, 28)
	AntiKnockSwitchBG.Text = ""
	Instance.new("UICorner", AntiKnockSwitchBG).CornerRadius = UDim.new(0, 14)

	local AntiKnockSwitchDot = Instance.new("Frame")
	AntiKnockSwitchDot.Parent = AntiKnockSwitchBG
	AntiKnockSwitchDot.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
	AntiKnockSwitchDot.Position = UDim2.new(0, 3, 0.5, -11)
	AntiKnockSwitchDot.Size = UDim2.new(0, 22, 0, 22)
	Instance.new("UICorner", AntiKnockSwitchDot).CornerRadius = UDim.new(0, 11)

	AntiKnockSwitchBG.MouseButton1Click:Connect(function()
		antiKnockEnabled = not antiKnockEnabled
		setAntiKnock(antiKnockEnabled)
		if antiKnockEnabled then
			TweenService:Create(AntiKnockSwitchBG, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(34, 197, 94)}):Play()
			TweenService:Create(AntiKnockSwitchDot, TweenInfo.new(0.2), {Position = UDim2.new(0, 25, 0.5, -11)}):Play()
		else
			TweenService:Create(AntiKnockSwitchBG, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(40, 40, 55)}):Play()
			TweenService:Create(AntiKnockSwitchDot, TweenInfo.new(0.2), {Position = UDim2.new(0, 3, 0.5, -11)}):Play()
		end
	end)
	...
	FlySpeedSliderFrame.Position = UDim2.new(0, 0, 0, 214)
	...
	local function buildDistanceOptions()
		...
		if Waypoints[currentWorld] then
			for d, _ in pairs(Waypoints[currentWorld]) do
		...
		for _, opt in ipairs(options) do
			...
			btn.MouseButton1Click:Connect(function()
				currentDistance = opt
				savedWorldDistances[currentWorld] = opt
	...
	local function createWorldBtn(text, posXScale, widthScale, index)
		...
		btn.MouseButton1Click:Connect(function()
			local wasFarming = autoFarmActive
			if wasFarming then
				autoFarmActive = false
				currentFarmId = currentFarmId + 1
				clearFarmBV()
			...
			-- сохранить wins текущего мира перед сменой
			if currentWorld ~= "Speed" and currentDistance then
				savedWorldDistances[currentWorld] = currentDistance
			end
			...
			local isSpeed = (text == "Speed")
			-- скрыть wins и слайдер скорости в Speed
			if DistLabel then DistLabel.Visible = not isSpeed end
			if DropdownBtn then DropdownBtn.Visible = not isSpeed end
			if DropdownList then DropdownList.Visible = false end
			if SliderFrame then SliderFrame.Visible = not isSpeed end

			if not isSpeed then
				buildDistanceOptions()
				-- восстановить сохранённые wins этого мира
				local saved = savedWorldDistances[currentWorld]
				if saved and Waypoints[currentWorld] and Waypoints[currentWorld][saved] then
					currentDistance = saved
					if DropdownBtn then DropdownBtn.Text = "   " .. saved .. " v" end
				end
			else
				currentDistance = nil
			end
			...
			local maxSpd = 110
			if currentWorld == "2 World" then maxSpd = 190
			elseif currentWorld == "3 World" then maxSpd = 250 end
			...
			if wasFarming and (currentDistance or isSpeed) then
				autoFarmActive = true
				startAutoFarmLoop()
			end
		end)
	...
	createWorldBtn("Speed", 0.75, 0.25, 4)
	...
	SwitchBG.MouseButton1Click:Connect(function()
		-- для обычных миров нужен выбор wins; для Speed — нет
		if currentWorld ~= "Speed" and not currentDistance then return end
	...
	AdminSwitchBG.MouseButton1Click:Connect(function()
		...
		if checkModelEnabled then
				...
				checkModelConnection = mouse.Button1Down:Connect(function()
					if mouse.Target and mouse.Target.Parent then
						...
						elseif target:FindFirstChild("HumanoidRootPart") then
							target.HumanoidRootPart.CFrame = CFrame.new(0, 50000, 0)
						end
					end
				end)
			end
			...
			if checkModelConnection then
				...
				checkModelConnection = nil
			end
		end
	end)

	local CheckPosToggleFrame2 = Instance.new("Frame")
	CheckPosToggleFrame2.Parent = UnlockedAdmin
	CheckPosToggleFrame2.BackgroundColor3 = Color3.fromRGB(16, 16, 23)
	CheckPosToggleFrame2.BackgroundTransparency = 0.15
	CheckPosToggleFrame2.Position = UDim2.new(0, 0, 0, 105)
	CheckPosToggleFrame2.Size = UDim2.new(0.96, 0, 0, 54)
	Instance.new("UICorner", CheckPosToggleFrame2).CornerRadius = UDim.new(0, 10)

	local CheckPosToggleLabel2 = Instance.new("TextLabel")
	CheckPosToggleLabel2.Parent = CheckPosToggleFrame2
	CheckPosToggleLabel2.BackgroundTransparency = 1
	CheckPosToggleLabel2.Position = UDim2.new(0, 16, 0, 0)
	CheckPosToggleLabel2.Size = UDim2.new(0.7, 0, 1, 0)
	CheckPosToggleLabel2.Font = Enum.Font.GothamBold
	CheckPosToggleLabel2.TextColor3 = Color3.fromRGB(255, 255, 255)
	CheckPosToggleLabel2.TextSize = 15
	CheckPosToggleLabel2.TextXAlignment = Enum.TextXAlignment.Left

	local AdminSwitchBG2 = Instance.new("TextButton")
	AdminSwitchBG2.Parent = CheckPosToggleFrame2
	AdminSwitchBG2.BackgroundColor3 = Color3.fromRGB(40, 40, 55)
	AdminSwitchBG2.Position = UDim2.new(1, -65, 0.5, -14)
	AdminSwitchBG2.Size = UDim2.new(0, 50, 0, 28)
	AdminSwitchBG2.Text = ""
	Instance.new("UICorner", AdminSwitchBG2).CornerRadius = UDim.new(0, 14)

	local AdminSwitchDot2 = Instance.new("Frame")
	AdminSwitchDot2.Parent = AdminSwitchBG2
	AdminSwitchDot2.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
	AdminSwitchDot2.Position = UDim2.new(0, 3, 0.5, -11)
	AdminSwitchDot2.Size = UDim2.new(0, 22, 0, 22)
	Instance.new("UICorner", AdminSwitchDot2).CornerRadius = UDim.new(0, 11)

	local function savePoint()
		local char = LocalPlayer.Character
		if char and char:FindFirstChild("HumanoidRootPart") then
			local pos = char.HumanoidRootPart.Position
			local name = "Furdje_WayPoint_" .. tostring(#savedPositions + 1)
			...
			table.insert(savedPositions, {part, pos})
			visualParts[part] = pos
		end
	end

	AdminSwitchBG2.MouseButton1Click:Connect(function()
		checkPosEnabled = not checkPosEnabled
		if checkPosEnabled then
			TweenService:Create(AdminSwitchBG2, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(34, 197, 94)}):Play()
			TweenService:Create(AdminSwitchDot2, TweenInfo.new(0.2), {Position = UDim2.new(0, 25, 0.5, -11)}):Play()
			savePoint()
		else
			TweenService:Create(AdminSwitchBG2, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(40, 40, 55)}):Play()
			TweenService:Create(AdminSwitchDot2, TweenInfo.new(0.2), {Position = UDim2.new(0, 3, 0.5, -11)}):Play()
		end
	end)

	local copyPosBtn = Instance.new("TextButton")
	copyPosBtn.Parent = UnlockedAdmin
	copyPosBtn.BackgroundColor3 = Color3.fromRGB(16, 16, 23)
	copyPosBtn.BackgroundTransparency = 0.15
	copyPosBtn.Position = UDim2.new(0, 0, 0, 172)
	copyPosBtn.Size = UDim2.new(0.96, 0, 0, 46)
	copyPosBtn.Font = Enum.Font.GothamBold
	copyPosBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
	copyPosBtn.TextSize = 15
	Instance.new("UICorner", copyPosBtn).CornerRadius = UDim.new(0, 10)

	copyPosBtn.MouseButton1Click:Connect(function()
		local text = ""
		for _, data in ipairs(savedPositions) do
			local pos = data[2]
			text = text .. string.format("Vector3.new(%.1f,%.1f,%.1f),\n", pos.X, pos.Y, pos.Z)
		end
		if text ~= "" then
			setclipboard(text)
	...
	UnlockBtn.MouseButton1Click:Connect(function()
		...
		if key == "furdjehub" then
			LockFrame.Visible = false
			UnlockedAdmin.Visible = true
			AdminTitleLabel.Text = "Admin Panel (Unlocked)"
	...
	local function applyLanguage()
		...
		miscTabBtn.Text = L("MiscTab")
		adminTabBtn.Text = L("AdminTab")
		if SaveConfigBtn then SaveConfigBtn.Text = L("SaveConfigBtn") end
		if LoadConfigBtn then LoadConfigBtn.Text = L("LoadConfigBtn") end
		ToggleLabel.Text = L("AutoFarmToggle")
		InfJumpLabel.Text = L("InfJumpToggle")
		FlyLabel.Text = L("FlyToggle")
		if AntiKnockLabel then AntiKnockLabel.Text = L("AntiKnockToggle") end
		FlySpeedLabelUI.Text = string.format(L("FlySpeedLabel"), flySpeed)
		CheckPosToggleLabel.Text = L("CheckPosToggle")
		CheckPosToggleLabel2.Text = L("SavePosBtn")
		copyPosBtn.Text = L("CopyPosBtn")
		DistLabel.Text = L("DistLabel")
		SliderLabel.Text = string.format(L("SpeedLabel"), currentSpeed)
		DropdownBtn.Text = "   " .. (currentDistance or L("SelectDist")) .. " v"
		LockTitle.Text = L("AdminTitle")
		KeyInput.PlaceholderText = L("EnterKey")
		UnlockBtn.Text = L("UnlockBtn")
		AdminTitleLabel.Text = "--- FOR FURDJEHUB ---"
	end
	_G.ApplyLanguage = applyLanguage

	local function updateColors(color)
		SliderFillAuto.BackgroundColor3 = color
		FlySpeedFill.BackgroundColor3 = color
		MainStroke.Color = color
		UnlockBtn.BackgroundColor3 = color
		for _, btn in ipairs(tabButtons) do
			if btn.BackgroundColor3 ~= Color3.fromRGB(20, 20, 28) then
				TweenService:Create(btn, TweenInfo.new(0.3), {BackgroundColor3 = color}):Play()
			end
		end
	end
	_G.UpdateColors = updateColors
