	...
	local function setAntiKnock(state)
		...
		if state then
			antiKnockConnection = RunService.Heartbeat:Connect(function()
				...
				-- во время автофарма/флая не мешаем
				...
				-- 1) снести чужие силы откидывания
				for _, obj in pairs(hrp:GetChildren()) do
					...
					if cn == "BodyVelocity" or cn == "BodyForce" or cn == "BodyPosition"
						...
						if obj.Name ~= "FurdjeFarmBV" and obj ~= flyBV
							and obj.Name ~= "FurdjeFlyBV" and obj.Name ~= "FurdjeFlyBG" then
							pcall(function() obj:Destroy() end)
						end
					end
				end

				-- 2) гасить только ОТКАТ (скорость против направления движения)
				--    вперёд / свой бег НЕ режем
				if hum and not autoFarmActive then
					local move = hum.MoveDirection
					if move.Magnitude > 0.2 then
						local vel = hrp.AssemblyLinearVelocity
						local horiz = Vector3.new(vel.X, 0, vel.Z)
						local against = -Vector3.new(move.X, 0, move.Z)
						if against.Magnitude > 0.1 then
							against = against.Unit
							local knock = horiz:Dot(against)
							-- сильный толчок назад от препятствия
							if knock > 40 then
								local cleaned = horiz - against * knock
								hrp.AssemblyLinearVelocity = Vector3.new(cleaned.X, vel.Y, cleaned.Z)
							end
						end
					end
				end
