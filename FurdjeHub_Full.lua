...
do
	...
	local Locales = {
		RU = {
			...
			MiscTab = "Config",
			ExtraMiscTab = "Misc",
			CollectGoldenKey = "Collect Golden Key",
			CollectSecretKey = "Collect Secret Key",
			AutoRebirthToggle = "Auto Rebirth",
	...
	local function L(key)
		...
		local en = Locales.EN
		if en and en[key] ~= nil then return en[key] end
		return tostring(key)
	end
	...
	local function startAutoFarmLoop()
		...
		task.spawn(function()
			-- ===== SPEED — ходьба вперёд-назад по полу (засчитывается) =====
			if farmWorld == "Speed" then
				...
				local function pressW(down)
					pcall(function()
						...
						vim:SendKeyEvent(down, Enum.KeyCode.W, false, game)
					end)
					-- fallback для некоторых executor
					pcall(function()
						if down then
				...
				local function faceTarget(target)
					...
					if (flat - pos).Magnitude < 0.1 then return end
					pcall(function()
						hrp.CFrame = CFrame.new(pos, flat)
					end)
				end
				...
				moveConn = RunService.RenderStepped:Connect(function()
					if not autoFarmActive or myId ~= currentFarmId or currentWorld ~= "Speed" then
					...
					if dist < 4 then
						...
						target = goingToB and pointB or pointA
						to = Vector3.new(target.X - pos.X, 0, target.Z - pos.Z)
						faceTarget(target)
					end
	...
	-- ===== Collect Keys =====
	local collectGoldenEnabled = false
	local collectSecretEnabled = false
	local goldenKeyConnection = nil
	local secretKeyConnection = nil
	...
	local function nameMatchesKey(name, patterns)
	...
	local GOLDEN_PATTERNS = {"golden key", "goldenkey", "gold key", "goldkey", "golden_key"}
	local SECRET_PATTERNS = {"secret key", "secretkey", "secret_key"}

	local function findKeyInWorkspace(patterns)
		for _, v in pairs(workspace:GetDescendants()) do
			if nameMatchesKey(v.Name, patterns) then
	...
	local function startKeyCollector(isGolden)
		local patterns = isGolden and GOLDEN_PATTERNS or SECRET_PATTERNS
		local enabledFlag = isGolden and function() return collectGoldenEnabled end or function() return collectSecretEnabled end

		task.spawn(function()
			local lastTp = 0
			while enabledFlag() do
				local part = findKeyInWorkspace(patterns)
				if part and tick() - lastTp > 0.8 then
					touchKey(part)
					lastTp = tick()
				end
				task.wait(0.35)
	...
	local function setCollectGolden(state)
		collectGoldenEnabled = state
		if state then startKeyCollector(true) end
	end

	local function setCollectSecret(state)
		collectSecretEnabled = state
		if state then startKeyCollector(false) end
	end

	-- ===== Auto Rebirth =====
	local autoRebirthEnabled = false
	local cachedRebirthRemotes = {}
	local lastRebirthScan = 0

	local function scanRebirthRemotes()
		cachedRebirthRemotes = {}
		local services = {}
		pcall(function() table.insert(services, game:GetService("ReplicatedStorage")) end)
		pcall(function() table.insert(services, game:GetService("ReplicatedFirst")) end)
		pcall(function()
			local pg = LocalPlayer:FindFirstChild("PlayerGui")
			if pg then table.insert(services, pg) end
		end)
		pcall(function() table.insert(services, LocalPlayer) end)
		...
		for _, root in ipairs(services) do
			...
			for _, obj in pairs(root:GetDescendants()) do
				...
				if (obj:IsA("RemoteEvent") or obj:IsA("RemoteFunction")) and (
					string.find(n, "rebirth", 1, true)
					or string.find(n, "prestige", 1, true)
					or string.find(n, "ascend", 1, true)
				) then
					table.insert(cachedRebirthRemotes, obj)
				end
			end
			end
		end
		-- часто в этих играх remote лежит глубже
		pcall(function()
			local rs = game:GetService("ReplicatedStorage")
			for _, folderName in ipairs({"Remotes", "Events", "Remote", "Packages", "Knit", "Controllers", "Systems"}) do
				local folder = rs:FindFirstChild(folderName)
				if folder then
					for _, obj in pairs(folder:GetDescendants()) do
						local n = string.lower(obj.Name or "")
						if (obj:IsA("RemoteEvent") or obj:IsA("RemoteFunction")) and string.find(n, "rebirth", 1, true) then
							local already = false
							for _, e in ipairs(cachedRebirthRemotes) do
								if e == obj then already = true break end
							end
							if not already then table.insert(cachedRebirthRemotes, obj) end
						end
					end
				end
			end
		end)
		lastRebirthScan = tick()
	end

	local function clickRebirthButtons()
		local function tryClick(gui)
			if not gui then return end
			for _, obj in pairs(gui:GetDescendants()) do
				if obj:IsA("TextButton") or obj:IsA("ImageButton") then
					local t = string.lower((obj.Text or "") .. " " .. obj.Name)
					if string.find(t, "rebirth", 1, true) or string.find(t, "перерожд", 1, true) then
						pcall(function()
		...
		pcall(function() tryClick(LocalPlayer:FindFirstChild("PlayerGui")) end)
		pcall(function() if gethui then tryClick(gethui()) end end)
		pcall(function() tryClick(game:GetService("CoreGui")) end)
	end

	local function fireRebirthRemotes()
		for _, remote in ipairs(cachedRebirthRemotes) do
			if remote and remote.Parent then
				if remote:IsA("RemoteEvent") then
					-- разные варианты аргументов
					pcall(function() remote:FireServer() end)
					pcall(function() remote:FireServer(true) end)
					pcall(function() remote:FireServer(1) end)
					pcall(function() remote:FireServer("Rebirth") end)
	...
	local function doRebirthOnce()
		if tick() - lastRebirthScan > 8 or #cachedRebirthRemotes == 0 then
			pcall(scanRebirthRemotes)
		end
		fireRebirthRemotes()
		clickRebirthButtons()
	end
	...
	local function buildLangButton(emoji, text, posX, langCode)
		...
		Btn.MouseButton1Click:Connect(function()
			lang = langCode
			pcall(function()
				if _G.ApplyLanguage then _G.ApplyLanguage() end
			end)
			pcall(function()
			...
			pcall(function() toggleMenu(true) end)
		end)
	end
	...
	local ExtraMiscPage = Instance.new("Frame")
	ExtraMiscPage.Parent = ContentArea
	ExtraMiscPage.BackgroundTransparency = 1
	ExtraMiscPage.Size = UDim2.new(1, 0, 1, 0)
	ExtraMiscPage.Visible = false
	...
	local function createTabButton(text, page)
		...
		btn.MouseButton1Click:Connect(function()
			...
			ExtraMiscPage.Visible = (page == ExtraMiscPage)
	...
	local extraMiscTabBtn = createTabButton("Misc", ExtraMiscPage)
	...
	local function createConfigToggle(parent, yPos, labelKey, key, getState, setState, onToggle)
		...
		Label.Text = tostring(L(labelKey) or labelKey)
	...
	-- ===== Extra Misc page (Keys) =====
	local ExtraMiscScroll = Instance.new("ScrollingFrame")
	ExtraMiscScroll.Parent = ExtraMiscPage
	ExtraMiscScroll.BackgroundTransparency = 1
	ExtraMiscScroll.Size = UDim2.new(1, 0, 1, 0)
	ExtraMiscScroll.ScrollBarThickness = 0
	ExtraMiscScroll.CanvasSize = UDim2.new(0, 0, 0, 340)

	local ExtraMiscTitle = Instance.new("TextLabel")
	ExtraMiscTitle.Parent = ExtraMiscScroll
	ExtraMiscTitle.BackgroundTransparency = 1
	ExtraMiscTitle.Position = UDim2.new(0, 0, 0, 0)
	ExtraMiscTitle.Size = UDim2.new(0.96, 0, 0, 28)
	ExtraMiscTitle.Font = Enum.Font.GothamBold
	ExtraMiscTitle.Text = "Misc"
	ExtraMiscTitle.TextColor3 = Color3.fromRGB(255, 255, 255)
	ExtraMiscTitle.TextSize = 16
	ExtraMiscTitle.TextXAlignment = Enum.TextXAlignment.Left

	createConfigToggle(ExtraMiscScroll, 36, "AutoRebirthToggle", "rebirth", function() return autoRebirthEnabled end, function(v) autoRebirthEnabled = v end, setAutoRebirth)
	createConfigToggle(ExtraMiscScroll, 104, "CollectGoldenKey", "golden", function() return collectGoldenEnabled end, function(v) collectGoldenEnabled = v end, setCollectGolden)
	createConfigToggle(ExtraMiscScroll, 172, "CollectSecretKey", "secret", function() return collectSecretEnabled end, function(v) collectSecretEnabled = v end, setCollectSecret)
	...
	_G.ApplyConfigToUI = function()
		pcall(function()
			...
			if _G.UpdateFarmModeUI then _G.UpdateFarmModeUI() end
	...
	local function updateFarmModeUI()
		local isSpeed = (currentWorld == "Speed")
		if DistLabel then DistLabel.Visible = not isSpeed end
		if DropdownBtn then DropdownBtn.Visible = not isSpeed end
		if DropdownList then DropdownList.Visible = false end
		if SliderFrame then SliderFrame.Visible = not isSpeed end
	end
	_G.UpdateFarmModeUI = updateFarmModeUI
	...
	local function applyLanguage()
		...
		if extraMiscTabBtn then extraMiscTabBtn.Text = L("ExtraMiscTab") end
	...
	pcall(applyLanguage)
	pcall(function() if _G.UpdateFarmModeUI then _G.UpdateFarmModeUI() end end)
	task.wait(0.3)
	LangFrame.Visible = true
	LangFrame.ZIndex = 100
	pcall(function()
		if LangScale then LangScale.Scale = 1 end
	end)
	ToggleWidget.Visible = false
	pcall(function() toggleMenu(false) end)
	-- меню откроется после выбора языка
end
